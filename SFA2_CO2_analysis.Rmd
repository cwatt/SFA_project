...
sample.df %>%
  filter(Sample != "foo") %>% # remove "foo" samples which are not data
  mutate(Control = if_else(Sample %in% c(379:381), "Y", "N")) %>% # distinguish negative controls and atmosphere sample
  filter(!str_detect(Sample, "blank")) %>% # remove blanks
  filter(!str_detect(Sample, "atmosphere")) %>% # remove atmospheric samples
...
```{r}
# NEED TO DOUBLE CHECK SOME STUFF
...
V = 0.013 # L, headspace of 30 mL serum bottle with sand ########### NEED TO CHECK##########
...
# Substract atmosphere from each sample
