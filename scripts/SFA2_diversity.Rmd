...
physeq_stat930 <- subset_samples(physeq_expstat, Day==9 | Day==30)

# Calculate weighted unifrac
wuni_stat930 <- distance(physeq_stat930, method="wunifrac")

# PCoA ordination
pcoa_stat930 <- ordinate(physeq_stat930, method="PCoA")
plot_ordination(physeq_stat930, pcoa_stat930, color="Day") +
  theme_test()
```
...
```{r}
meta_stat930 <- data.frame(sample_data(physeq_stat930))
adonis(wuni_stat930 ~ Day, data=meta_stat930, strata=meta_stat930$Inoculant, permutations = 9999)
```
...
ggplot(data=pcoa030_df, aes(x=Axis.1, y=Axis.2, color=Day, shape=Day)) +
  geom_path(data=df_ell, aes(x=x, y=y, colour=Day), size=1, linetype=2) +
  geom_point() +
  theme_test()
...
Supplemental beta div day 9 vs day 30:
...
# Extract ordination from pcoa class object
pcoa930_df <- pcoa_stat930$vectors %>% 
  data.frame() %>% 
  select(Axis.1, Axis.2) %>% 
  rownames_to_column(var="SampleID") %>% 
  left_join(meta)
...
library(ellipse)
df_ell <- data.frame()
for(d in unique(pcoa930_df$Day)){
    df_ell <- rbind(df_ell, cbind(as.data.frame(with(pcoa930_df[pcoa930_df$Day==d,],
                                           ellipse(cor(Axis.1, Axis.2),
                                                           scale=c(sd(Axis.1),sd(Axis.2)),                                                                   centre=c(mean(Axis.1), mean(Axis.2)))
                                           )), Day=d))
}
...
# Visualize
pcoa930_df <- pcoa930_df %>% 
  mutate(Day=as.character(Day))

df_ell <- df_ell %>% 
  mutate(Day=as.character(Day))

ggplot(data=pcoa930_df, aes(x=Axis.1, y=Axis.2, color=Day, shape=Day)) +
  geom_path(data=df_ell, aes(x=x, y=y, colour=Day), size=1, linetype=2) +
  geom_point() +
  theme_test()
```
