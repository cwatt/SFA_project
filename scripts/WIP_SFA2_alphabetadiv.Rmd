...
# Growth vs headspace communities
...
# High only
physeq_hilow_high <- subset_samples(physeq_rare_hilow, DOC=="high")
high_dist <- distance(physeq_hilow_high, method="wunifrac")
high_ord <- ordinate(physeq_hilow_high, distance=high_dist, method="NMDS")

plot_ordination(physeq_hilow_high, high_ord, color="Type") +
  stat_ellipse(type="t") +
  labs(title="High DOC") +
  theme_test() +
  theme(title = element_text(size=18),
...
# Low only
physeq_hilow_low <- subset_samples(physeq_rare_hilow, DOC=="low")
low_dist <- distance(physeq_hilow_low, method="wunifrac")
low_ord <- ordinate(physeq_hilow_low, distance=low_dist, method="NMDS")

plot_ordination(physeq_hilow_low, low_ord, color="Type") +
  stat_ellipse(type="t") +
  labs(title="Low DOC") +
  theme_test() +
  theme(title = element_text(size=18),
...
relabund <- count_relabund %>% 
  group_by(DOC, Type, Inoculant, ASV) %>% # average across technical replicates
...
```{r}
# Isolate controls and calculate relative abundance
physeq_control <- subset_samples(physeq_rare_end, is.na(DOC))
physeq_control_relabund <- transform_sample_counts(physeq_control, function(x) x/sum(x))

# Export from phyloseq
meta_ctrl <- data.frame(sample_data(physeq_control_relabund)) %>% 
  rownames_to_column(var="merge") %>% 
  select(merge, Sample, Type, Inoculant, DOC, Replicate)

count_ctrl <- as.matrix(otu_table(physeq_control_relabund)) %>% 
  t() %>% 
  as.data.frame() %>%
  rownames_to_column(var="merge") %>% 
  inner_join(meta_ctrl) %>% 
  pivot_longer(c(everything(), -c(merge, Sample, Type, Inoculant, DOC, Replicate)),
               names_to="ASV", values_to="relabund")

tax_ctrl <- as.data.frame(tax_table(physeq_end_relabund)) %>% 
  rownames_to_column(var="ASV")
  
relabund_ctrl <- count_ctrl %>% 
  group_by(DOC, Type, Inoculant, ASV) %>% # average across technical replicates
  summarize(relabund = mean(relabund)) %>% 
  ungroup() %>% 
  inner_join(tax_ctrl)
