...
# Prepare data
growth_slim <- growth %>% 
  select(Inoculant, ASV, k, start_day, end_day, n16S)
...
exp_gain1 <- filter(growth_norm, end_day <= 9) %>% # grew only during exponential
  filter(start_day==Day) %>% # abundance at start of growth (N0)
...
death_norm <- norm %>% 
  ...
  mutate(norm_abund_corr = norm_abund/n16S, # normalized abundance corrected
         h = log(2)/abs(k)) %>%  # halving time
  select(Inoculant, Day, ASV, start_day, end_day, h, norm_abund_corr)
```
...
# Group 1
exp_loss1 <- filter(death_norm, end_day <= 9) %>% # died only during exponential
  filter(start_day==Day) %>% # abundance at start of death (N0)
  mutate(deltaN = norm_abund_corr*2^(abs(end_day-start_day)/h) - norm_abund_corr) %>% # lost during death
  group_by(Inoculant) %>% 
  summarize(deltaN = sum(deltaN)) %>% 
  ungroup()

# Group 2
exp_loss2 <- filter(death_norm, end_day >= 9 & start_day < 9) %>% # died after exponential too
  filter(start_day==Day) %>% # starting abundance will be start day
  mutate(deltaN = norm_abund_corr*2^(abs(9-start_day)/h) - norm_abund_corr) %>% # lost prior to day 9 
  group_by(Inoculant) %>% 
  summarize(deltaN = sum(deltaN)) %>% 
  ungroup() %>% 
  add_row(Inoculant="41", deltaN=0) # missing incoulants
...
stat_loss1 <- filter(death_norm, end_day >= 9 & start_day <= 9) %>% # started growing in exponential
  filter(Day==9) %>% # starting abundnace will be abundance on day 9
  mutate(deltaN = norm_abund_corr*2^(abs(end_day-9)/h) - norm_abund_corr) %>% # lost after day 9
  group_by(Inoculant) %>% 
  summarize(deltaN = sum(deltaN)) %>% 
  ungroup() %>% 
  add_row(Inoculant="41", deltaN=0) # missing incoulants
...
# Group 3
stat_loss2 <- filter(death_norm, end_day >= 9 & start_day >= 9) %>% # started dying in stationary
  filter(Day==start_day) %>% # abundance at start of death
  mutate(deltaN = norm_abund_corr*2^(abs(end_day-start_day)/h) - norm_abund_corr) %>%
  group_by(Inoculant) %>% 
  summarize(deltaN = sum(deltaN)) %>% 
  ungroup() %>% 
  add_row(Inoculant="47", deltaN=0) %>% # add missing incoulants (no observed estiamtes)
  add_row(Inoculant="10", deltaN=0)
