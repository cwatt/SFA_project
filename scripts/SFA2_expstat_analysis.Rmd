...
plot_bin1_ex <- norm_bin1_ex %>% 
  ...
  labs(title=paste0("Bin 1 example - ", growth_bin1_ex$Phylum, ", ", growth_bin1_ex$Genus), x="Day", y="ln normalized abundance") +
  coord_cartesian(xlim = c(0, 30)) +
  theme_test()
plot_bin1_ex
```
...
# Visualize
plot_bin2_ex <- norm_bin2_ex %>% 
  ggplot(aes(x=Day, y=log(norm_abund_avg))) +
  geom_point(shape=1, size=3, color="#6F7378") +
  geom_line(color="#6F7378") +
  geom_vline(xintercept=9, linetype=2) +
  geom_smooth(method="lm", data=norm_bin2_ex[growth_bin2_ex$start_pt:growth_bin2_ex$end_pt,], linetype=2, color="black") +
  labs(title=paste0("Bin 2 example - ", growth_bin2_ex$Phylum, ", ", growth_bin2_ex$Genus), x="Day", y="ln normalized abundance") +
  coord_cartesian(xlim = c(0, 30)) +
  theme_test()
plot_bin2_ex
```
...
```{r}
# Bin 3
growth_bin3_ex <- growth_bin %>% 
  filter(ASV==growth[162,]$ASV & Inoculant==growth[162,]$Inoculant) # see growth estimation plots from script 03

# Prepare abudnance table
norm_bin3_ex <- norm_meta %>%
  filter(ASV==growth[162,]$ASV & Inoculant==growth[162,]$Inoculant) %>% 
  group_by(Inoculant, Day) %>% 
  summarize(norm_abund_avg = mean(norm_abund, na.rm=TRUE)) %>% # average across technical replicates
  ungroup() %>% 
  arrange(Day) %>% 
...
# Visualize
plot_bin3_ex <- norm_bin3_ex %>% 
  ggplot(aes(x=Day, y=log(norm_abund_avg))) +
  geom_point(shape=1, size=3, color="#6F7378") +
  geom_line(color="#6F7378") +
  geom_vline(xintercept=9, linetype=2) +
  geom_smooth(method="lm", data=norm_bin3_ex[growth_bin3_ex$start_pt:growth_bin3_ex$end_pt,], linetype=2, color="black") +
...
```{r}
# Combine examples into one df
norm_bin1_ex <- add_column(norm_bin1_ex, bin="bin1")
norm_bin2_ex <- add_column(norm_bin2_ex, bin="bin2")
norm_bin3_ex <- add_column(norm_bin3_ex, bin="bin3")

norm_bins_ex <- bind_rows(norm_bin1_ex, norm_bin2_ex) %>% 
  bind_rows(norm_bin3_ex)
