...
```{r}
# All
cor.test(growth_grp$length, log(growth_grp$change_abund_corr), method="pearson")

# Group 2 only
cor.test(growth_grp[growth_grp$group=="grp2",]$length, log(growth_grp[growth_grp$group=="grp2",]$change_abund_corr), method="pearson")
```
...
```{r}
# All
cor.test(growth_grp$k, log(growth_grp$change_abund_corr), method="pearson")
...
```{r}
plot_length1 <- growth_grp %>% 
  ggplot(aes(x=length, y=log(change_abund_corr))) + #, color=group
  geom_point(aes(shape=group)) +
  geom_smooth(method="lm", linetype=2, color="#444444", alpha=0.5) +
  theme_test() +
  theme(title = element_text(size=12),
        axis.title = element_blank(),
          axis.text = element_text(size=10),
...
plot_k1 <- growth_grp %>% 
  ggplot(aes(x=k, y=log(change_abund_corr))) + #, color=group
  geom_point(aes(shape=group)) +
  geom_smooth(method="lm", linetype=2, alpha=0.5, color="#444444") +
  theme_test() +
  theme(title = element_text(size=12),
        axis.title = element_blank(),
        axis.text = element_text(size=10),
...
```{r}
# Combine
plot_lengthk <- plot_grid(plot_length1, plot_length2, plot_k1, plot_k2, nrow=2)
plot_lengthk
```
