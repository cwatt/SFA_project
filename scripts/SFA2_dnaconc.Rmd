...
Figure for SFA progress report:

```{r}
dna.plot <- dna.avg %>%
  mutate(Innoculant = as.factor(Innoculant),
         DOC_pheno = fct_recode(DOC_pheno, 'DOC phenotype: high'="high", 'DOC phenotype: low'="low")) %>%
  filter(Day < 30) %>%
  ggplot(aes(x=Day, y=dna_avg)) +
  geom_point(aes(color=Innoculant)) +
  geom_errorbar(aes(ymin=dna_avg-dna_sd, ymax=dna_avg+dna_sd), color="gray") +
  geom_line(aes(color=Innoculant)) +
  facet_wrap(~DOC_pheno) +
  labs(y="Mean DNA conc. (ng/uL)") +
  theme_test() +
...
ggsave(dna.plot, file="../figures/SFA2_dnaconc.png", units="mm", width=200, height=100)
```
