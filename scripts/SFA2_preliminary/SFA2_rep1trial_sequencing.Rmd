...
```{r, eval=FALSE}
# Subtract total reads form aquifex reads
reads <- reads %>%
  mutate(reads_nospike = total_reads - aquifex_reads) %>%
  filter(Type == "Growth")

min(reads$reads_nospike)
max(reads$reads_nospike)
mean(reads$reads_nospike)
median(reads$reads_nospike)
```
...
Aquifex spike-in ASVs
---------------------

Why are there so many ASVs for Aquifex when I only included one sequence? DADA2 is up to something...
