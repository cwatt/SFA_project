...
norm_death <- readRDS("../data_intermediate/SFA2_norm_deestimated.rds")
...
In the low DOC treatment. I think these few ASVs could be contributing to a trend of more biomass gain overall in low treatment.

```{r}
outliers <- readRDS("../data_intermediate/SFA2_growth_changeabundlow_outliers.rds")
outlier_labels <- outliers$label
outlier_asvs <- outliers$ASV
...
norm_label %>% 
  mutate(target = if_else(label %in% outlier_labels, "yes", "no")) %>% 
  ggplot(aes(x=Day, y=norm_abund_avg, color=Phylum, alpha=target)) +
  geom_line(aes(alpha=target)) +
  geom_point() +
  facet_wrap(DOC ~ Inoculant) +
  theme_test()
...
norm_label %>% 
  mutate(target = if_else(label %in% outlier_labels, "yes", "no")) %>% 
  ggplot(aes(x=Day, y=ln_norm_abund_avg, color=Phylum, alpha=target)) +
  geom_line(aes(alpha=target)) +
  geom_point() +
  facet_wrap(DOC ~ Inoculant) +
  theme_test()
...
```{r}
norm_label %>% 
  mutate(target = if_else(ASV %in% outlier_asvs, "yes", "no")) %>% 
  ggplot(aes(x=Day, y=norm_abund_avg, color=Phylum, alpha=target)) +
  geom_line(aes(alpha=target)) +
  geom_point() +
  facet_wrap(DOC ~ Inoculant) +
