...
```{r}
growth <- readRDS("~/SFA/data_intermediate/SFA2_growth_estimates_pap.rds")

death <- readRDS("~/SFA/data_intermediate/SFA2_death_estimates_pap.rds")
```
...
growth_prep <- growth %>% 
  ...
  mutate(start_abund = start_abund/n16S, # Correct for 16S copy number
         end_abund = end_abund/n16S,
         change_abund =change_abund/n16S,
         length = end_day - start_day) %>%  # add length parameter
  select(k, start_day, end_day, length, start_abund, end_abund, change_abund)
...
End abundance and change abundance are almost perfectly correlated (0.999), so I'll remove end_abund, since change in abundance is a particular parameter of interest. Length and end abundance are highly correlated, so I'll remove length.

```{r}
# Remove unneeded parameters, reformat
growth_prep <- growth_prep %>% 
  select(-end_abund, -length)
```
...
```{r, eval=FALSE}
saveRDS(growth_clusters, "../data_intermediate/SFA2_growthclusters.Rmd")
```
