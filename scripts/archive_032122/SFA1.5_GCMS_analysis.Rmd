...
# Summary statistics
stds.summary.df <- stds.nona.df %>% 
  group_by(std) %>% 
  summarise(mean = mean(area.conv), sd=sd(area)) %>% 
  ungroup()
...
for (i in stds.summary.df$std) {
  x <- filter(stds.nona.df[stds.nona.df$std==i,], !(area.conv < stds.summary.df$cutoff_min[stds.summary.df$std==i] | 
                                                      area.conv > stds.summary.df$cutoff_max[stds.summary.df$std==i])) # remove outliers
...
# Sum all time points
totalresp.df <- samples.df %>% 
  group_by(sample, rep) %>% 
  summarize(CO2.total = sum(CO2mg, na.rm=TRUE)) %>% 
  ungroup()
...
```{r, eval=FALSE}
ggsave(resp_plot, file="../figures/SFA1.5_CO2.png", units="mm", width=100, height=100)
```

```{r}
co2_lm <- lm(CO2.mean ~ DOC, data=totalresp.summary)
hist(resid(co2_lm))
plot(predict(co2_lm), resid(co2_lm))

# Use test that can handle unequal variances = Welch's t-test
t.test(CO2.mean ~ DOC, data = filter(totalresp.summary, DOC!="negative"), var.equal=FALSE)
```
