...
```{r}
gr_change_lm <- lm(log(change_abund) ~ Succession, growth_succ_overall)
hist(resid(gr_change_lm))
plot(predict(gr_change_lm), resid(gr_change_lm))
summary(gr_change_lm)
```
...
```{r}
# Average of each inoculant
death_after_overall <- growth_succ %>% 
  group_by(Inoculant, Succession) %>%
  summarize(k = mean(k),
            start_abund = mean(start_abund),
            end_abund = mean(end_abund),
...
```{r}
gr_change_lm <- lm(log(abs(change_abund)) ~ Succession, death_after_overall)
hist(resid(gr_change_lm))
plot(predict(gr_change_lm), resid(gr_change_lm))
summary(gr_change_lm)
```
...
```{r}
gr_length_lm <- lm(length ~ Succession, death_after_overall)
hist(resid(gr_length_lm)) # non normal
plot(predict(gr_length_lm), resid(gr_length_lm))
...
plot(predict(genome_lm), resid(genome_lm)) # heteroskedastic

# Use Welch Anova
oneway.test(genome_size ~ Succession, genome_overall)
```
