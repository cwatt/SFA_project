...
redo_lowstd <- std_depth_rich %>% 
  mutate(std_prop = std_count/read_depth) %>%
  filter(std_count < 35 & std_count > 0) %>% 
  .$Sample
...
Look at distributions of read counts for ASVs

* What proportion of ASVs in each sample have less than 30 reads?
...
asvs_rarefied <- tax_rarefied %>% 
  rownames_to_column(var="ASV") %>% 
  .$ASV

# Remove taxa from non-rarefied data that did not pass rarefication
physeq5 <- prune_taxa(asvs_rarefied, physeq3)

physeq5
```

How many samples contain no internal standard after rarefying?

```{r}
count_rarefied <- data.frame(otu_table(physeq4))

std_rarefied <- count_rarefied %>% 
  rownames_to_column(var="ASV") %>% 
  filter(ASV == "445e8681c3c1a735760e6c394f5f4d0a") %>% 
  select(-ASV) %>%
  gather(variable, value) %>% 
...
read_depth2 <- count_processed2 %>% 
  ...
  gather(Sample, count, -ASV) %>% 
  spread(ASV, count) %>% 
  mutate(read_depth = rowSums(.[-1])) %>% 
  select(Sample, read_depth)
...
# Merge together, add metadata
depth_rich_processed2 <- inner_join(read_depth2, richness2, by="Sample") %>% 
  inner_join(meta_processed, by="Sample") %>% 
  select(c(Sample, read_depth, richness = `.`, Type:PCR_plate)) %>% 
  mutate(Replicate = as_factor(Replicate))

depth_rich_processed2 %>% 
  ggplot(aes(x=read_depth, y=richness, color=Replicate)) +
  geom_point() +
  theme_test()

depth_rich_processed2 %>% 
  ggplot(aes(x=read_depth, y=richness, color=Day)) +
