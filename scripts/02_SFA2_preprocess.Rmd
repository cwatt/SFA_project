...
```{r, eval=FALSE}
saveRDS(meta, file="../data_intermediate/SFA2_metadata.rds")
```
...
meta_fix <- meta %>% 
  ...
  select(SampleID, Sample:Primer_num)
...
reseq_compare <- inner_join(read_depth_reseq, spike_counts) %>% 
  inner_join(richness) %>% 
  select(c(SampleID, Sample, Type:Primer_num, total_reads, std_count, richness=`.`)) %>% 
  mutate(Replicate = as_factor(Replicate))
```
...
I'll rarefy to 1500 reads and lose 38 samples. Want to preserve more counts.

```{r}
set.seed(21)
physeq_rare_div <- rarefy_even_depth(physeq6, sample.size=1500)
physeq_rare_div
```

### For bNTI

I'll rarefy to 500. This is the lowest rarefication rate I've seen used with this analysis and I'd like to preserve the greatest number of samples

```{r}
physeq_rare_bnti <- rarefy_even_depth(physeq6, sample.size = 500)
physeq_rare_bnti
```

# Save preprocessed count data

Rarefied and unrarefied versions, without internal standard.

```{r, eval=FALSE}
# Rarefied for diversity analysis
saveRDS(physeq_rare_div, file="../data_intermediate/SFA2_physeq_rare.rds")

# Rarefied for bNTI
saveRDS(physeq_rare_bnti, file="../data_intermediate/SFA2_physeq_rare_bnti.rds")

# Not rarefied
saveRDS(physeq6, file="../data_intermediate/SFA2_physeq_unrare.rds")
```
...
norm_meta <- count_norm %>% 
  ...
  select(c(SampleID, Sample, Type:Replicate, norm_abund, ASV, Domain:Species)) %>% 
...
saveRDS(norm_meta, file="../data_intermediate/SFA2_normalized.rds")
```
