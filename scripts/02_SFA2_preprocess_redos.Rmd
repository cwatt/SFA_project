...
count_long <- count %>% 
  pivot_longer(cols=-ASV, names_to = "SampleID", values_to = "count") %>% 
  mutate(SampleID = as.double(SampleID))

tax <- full_join(tax_rep1, tax_rep2) %>%
  full_join(tax_rep3) %>% 
  full_join(tax_redos)

sfa2_raw <- full_join(meta, count_long) %>% 
  full_join(tax)
```

### Remove non-prokaryotic ASVs

Chloroplast, mitochondria, eukarya, etc.

```{r}
# Remove non-prokaryotic ASVs
sfa2_taxfilt <- filter(sfa2_raw, Order != "Chloroplast" | 
                         Family != "Mitochondria" | 
                         Domain != c("Unassigned", "putative Unassigned", "putative Bacteria"))
...
sfa2_sparcefilt <- filter(sfa2_taxfilt, ASV %in% sparce_pass_asvs)
