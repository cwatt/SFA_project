...
# Community dissimilarity vs growth efficiency

```{r}
# Combine with metadata for Sample1
meta_short <- meta %>% 
  select(SampleID, ucosm=Sample, Inoculant, Day, Type)

wuni_meta <- inner_join(wuni_dist, meta_short, by=c("Sample1"="SampleID")) %>%
  select(Sample1, Sample2, Inoc1=Inoculant, Day1=Day, wuni_dist) %>% 
  inner_join(meta_short, by=c("Sample2"="SampleID")) %>%
  select(Sample1, Sample2, Inoc1, Inoc2=Inoculant, Day1, Day2=Day, wuni_dist)

# Filter non-self distance comparisons and find centroid
wuni_noself <- wuni_meta %>% 
  mutate(lab1 = paste0(Inoc1, "_", Day1),
         lab2 = paste0(Inoc2, "_", Day2)) %>% 
  filter(lab1!=lab2) %>% # remove comparisons that are not between the same Inoculant and day
  group_by(lab1) %>% 
  summarize(wuni_mean = mean(wuni_dist))

# Calculate distances between inoculants at each time point
days <- filter(meta, Day <= 30 & !is.na(Day))
days <- unique(days$Day)
inocs <- filter(meta, Inoculant!=51 & !is.na(Inoculant))
inocs <- unique(inocs$Inoculant)

ffuu <- data.frame()

for (d in days) {
    data_filt <- filter(wuni_noself, Day1==d & Day2==d)
    for (i in 1:length(inocs)) {
      inoc <- inocs[i]
      data_filt2 <- filter(data_filt, Inoc1==inoc)
      if (i < 6) {
        for (x in 1:(6-i)) { # does this even do what I think it does?
        inoc_next <- inocs[i+x]
        data_filt3 <- filter(data_filt2, Inoc2==inoc_next)
        dist_mean <- mean(data_filt3$wuni_dist)
        this_row <- bind_cols(Inoc1=inocs[i], Inoc2=inocs[x], Day=d, wuni_mean=dist_mean)
        ffuu <- bind_rows(ffuu, this_row)
        }
      }
      else {
          next
