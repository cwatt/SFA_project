...
growth_impute <- growth_resptps %>% 
  ...
  mutate(growing = if_else(Day >= start_day & Day <= end_day, "yes", "no"), # label if during growth period
         impute_abund = if_else(growing=="yes", slope*Day+yint, NA), # impute using linear estimate
...
```{r}
greff_phase <- greff_df %>% 
  mutate(phase = if_else(Day < 9, "exponential", "stationary")) %>% 
  group_by(phase, Inoculant) %>% 
  summarize(greff = mean(greff))

greff_phase

greff_phase %>% 
  ggplot(aes(x=phase, y=log(greff))) +
  geom_point() + 
  theme_test()

# Statistics
greffphase_lm <- lm(log(greff) ~ phase, data=greff_phase)
hist(resid(greffphase_lm)) # normalish
plot(predict(greffphase_lm), resid(greffphase_lm)) # heteroskedastic
# welch t test
t.test(log(greff) ~ phase, data=greff_phase, var.equal=FALSE)
```
