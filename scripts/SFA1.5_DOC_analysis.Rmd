...
doc.plot <- doc.mean %>% 
  ...
  #annotate("text", x=2.1, y=1350, label="P-value < 0.0001") +
  theme_test() +
...
plot(predict(doc.lmer), residuals(doc.lmer))
summary(doc.lmer)
```
...
Data appears normal and has equal variances.

```{r}
# Mixed linear model
# Innoculant as random effect
doc_lmer = lmer(mean ~ DOC + (1|Sample), data = filter(doc.meta, DOC != "negative"))
hist(resid(doc_lmer))
plot(predict(doc_lmer), resid(doc_lmer))
summary(doc_lmer)
