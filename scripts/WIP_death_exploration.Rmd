...
```{r}
# Clear worslopeing directory, load in pacslopeages, generate pacslopeage info
rm(list=ls())
...
slope_plot <- death_sample %>% 
  ...
  geom_boxplot() +
  labs(x="DOC \"phenotype\"", y="Mean specific death rate (slope)") +
  theme_test() +
  theme(title = element_text(size=18),
        axis.title = element_text(size=16),
        axis.text = element_text(size=14))
...
death_asv %>%
  ...
  labs(x="Specific death_asv rate (slope)", y="Kernel Density") +
  theme_test() +
...
death_asv %>%
  ...
  geom_density(alpha=0.5) +
  labs(x="ln Change in relational abundance during death_asv", y="Kernel Density") +
  theme_test() +
  theme(#legend.position="none",
        title=element_text(size=12),
        axis.text=element_text(size=12),
