...
# PAPRICA outputs
# Bacteria
pap_bac <- read_csv("../paprica_analysis/asvs_all/SFA2_full.rep-seqs-merged.bacteria.edge_data.csv")
place_bac <- read_csv("../paprica_analysis/asvs_all/SFA2_full.rep-seqs-merged.bacteria.combined_16S.bacteria.tax.placements.csv")

# Archaea
pap_arc <- read_csv("../paprica_analysis/asvs_all/SFA2_full.rep-seqs-merged.archaea.edge_data.csv")
place_arc <- read_csv("../paprica_analysis/asvs_all/SFA2_full.rep-seqs-merged.archaea.combined_16S.archaea.tax.placements.csv")

# Growth and death estimates
growth <- readRDS("../data_intermediate/SFA2_growth_estimates2.rds")
death <- readRDS("../data_intermediate/SFA2_death_estimates2.rds")

# Taxonomy
tax <- readRDS("../data_intermediate/SFA2_taxonomy.rds")

# Pathways
path_bac <- read_csv("../paprica_analysis/asvs_all/SFA2_full.rep-seqs-merged.archaea.pathways.csv")
path_arc <- read_csv("../paprica_analysis/asvs_all/SFA2_full.rep-seqs-merged.bacteria.pathways.csv")
```
...
acido_n16S <- as.numeric(missing_impute[1,2])
acido_genome <- as.numeric(missing_impute[1,3])
acido_phi <- as.numeric(missing_impute[1,4])
acido_npaths <- as.numeric(missing_impute[1,5])
acido_nec <- as.numeric(missing_impute[1,6])

# Chloroflexi imputations
chloro_n16S <- as.numeric(missing_impute[2,2])
chloro_genome <- as.numeric(missing_impute[2,3])
chloro_phi <- as.numeric(missing_impute[2,4])
chloro_npaths <- as.numeric(missing_impute[2,5])
chloro_nec <- as.numeric(missing_impute[2,6])
...
```{r}
growth_pap <- left_join(growth, pap_impute, by="ASV") %>% 
  select(-global_edge_num)

death_pap <- left_join(death, pap_impute, by="ASV") %>% 
  select(-global_edge_num)
```
...
```{r, eval=FALSE}
saveRDS(pap_impute, "../data_intermediate/SFA2_paprica_all2.rds")
