...
Yes, how do their change in abundances compare between treatments?
...
```{r}
out_chabund_lm <- lm(log(change_abund) ~ DOC, filter(growth, ASV %in% out_asvs))
hist(resid(out_chabund_lm))  
plot(predict(out_chabund_lm), resid(out_chabund_lm))
summary(out_chabund_lm)
```
