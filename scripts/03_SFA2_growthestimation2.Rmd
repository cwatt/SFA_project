...
```{r, eval=FALSE}
saveRDS(norm_tidy, file="../data_intermediate/SFA2_norm_grestimated2.rds")
```
...
growth_tidy <- g.df %>% 
  ...
  select(label, Inoculant, ASV, slope:yint, k, g, start_pt:change_abund)
```

```{r, eval=FALSE}
saveRDS(growth_tidy, file="../data_intermediate/SFA2_growth_estimates2.rds")
```
...
write_tsv(growth_asvs, file="../paprica_analysis/growth_asvs2.txt")
```
