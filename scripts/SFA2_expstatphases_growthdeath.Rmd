...
# Identify pre and post level off
...
# change in abundance
growth_prepost %>% 
  ggplot(aes(x=startend, y=log(change_abund_corr), color=startend)) +
  geom_boxplot() +
  geom_jitter(size=2) +
  theme_test() +
...
# change abund
chg_lmer <- lmer(log(change_abund_corr) ~ startend + (1 + Inoculant | Inoculant), growth_prepost)
hist(resid(chg_lmer))
...
chg_lmer2 <- lmer(log(change_abund_corr) ~ startend + (1 + Inoculant | Inoculant), growth_prepost_nosteady)
...
# change in abundance
death_prepost %>% 
  ggplot(aes(x=startend, y=log(abs(change_abund_corr)), color=startend)) +
  geom_boxplot() +
  geom_jitter(size=2) +
  theme_test() +
  theme(axis.text = element_text(size = 12),
...
# change abund
chgdeath_lmer <- lmer(log(abs(change_abund_corr)) ~ startend + (1 + Inoculant | Inoculant), death_prepost)
